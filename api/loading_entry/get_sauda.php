...
try {
    ...
    if ($entryNo <= 0) {
        throw new Exception('Sauda number is required');
    }
    if ($bookCode === '') {
        $bookCode = 'SUD';
    }
    ...
    $headerStmt = mysqli_prepare(
        ...
        "SELECT sno, DATE_FORMAT(d_a_t_e, '%Y-%m-%d') AS entry_date, c_j_s_p, vouc_chr, vouc_code,
                pcd AS buyer_id, party_name AS buyer_name, scode AS seller_id, bbcode AS sub_broker_id, load_req, rmks
         FROM etrans1
    ...
    if (!$header) {
        throw new Exception('Sauda not found');
    }
    ...
    $itemStmt = mysqli_prepare(
        ...
        "SELECT sno, sr_no, it_code AS product_id, item_name AS product_name,
                brnd_code AS brand_id, brand_name, qty, pck, wght, rate, amount, ratetyp, nar, pend_qty, oppcode
         FROM etrans2
         WHERE etrans1_sno = ? AND main_bk = 'SDBYR' AND del = 'N' AND pend_qty > 0
         ORDER BY sr_no, sno"
    );
    ...
    while ($itemRes && ($row = mysqli_fetch_assoc($itemRes))) {
        $items[] = [
            'source_detail_sno' => (int)($row['sno'] ?? 0),
            'seller_detail_sno' => (int)($row['oppcode'] ?? 0),
            'sr_no' => (int)($row['sr_no'] ?? 0),
            'product_id' => (int)($row['product_id'] ?? 0),
            'product_name' => (string)($row['product_name'] ?? ''),
            'brand_id' => (int)($row['brand_id'] ?? 0),
            'brand_name' => (string)($row['brand_name'] ?? ''),
            'source_qty' => (float)($row['qty'] ?? 0),
            'pending_qty' => (float)($row['pend_qty'] ?? 0),
            'qty' => 0,
    ...
    echo json_encode([
        ...
        'header' => [
            ...
            'book_code' => (string)($header['c_j_s_p'] ?? 'SUD'),
