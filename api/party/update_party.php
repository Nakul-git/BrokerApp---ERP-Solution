...
function clear_related($con, $party_id, $user_id) {
...
$party_id = post_int('party_id', 0);
$party_name = post_str('party_name');
$city = post_str('city');
if ($party_id <= 0 || $party_name === '' || $city === '') {
    echo json_encode(["status" => "error", "message" => "Invalid data"]);
    exit;
}

$dup = mysqli_prepare($con, "SELECT party_id FROM party WHERE user_id=? AND LOWER(party_name)=LOWER(?) AND LOWER(city)=LOWER(?) AND party_id<>? LIMIT 1");
mysqli_stmt_bind_param($dup, "issi", $user_id, $party_name, $city, $party_id);
...
foreach ($field_map as $col => $meta) {
    ...
    $set[] = $col . "=" . sql_val($con, $meta['v'], $meta['t']);
}

mysqli_begin_transaction($con);
try {
    $sql = "UPDATE party SET " . implode(", ", $set) . " WHERE party_id=" . (int)$party_id . " AND user_id=" . (int)$user_id;
    if (!mysqli_query($con, $sql)) throw new Exception('Failed to update party');
    clear_related($con, $party_id, $user_id);
    save_related($con, $party_id, $user_id);
    mysqli_commit($con);
    echo json_encode(["status" => "success", "message" => "Party updated"]);
} catch (Throwable $e) {
    mysqli_rollback($con);
