...
$dup = mysqli_prepare(
    $con,
    "SELECT district_id FROM district WHERE user_id=? AND state_id=? AND LOWER(district_name)=LOWER(?) LIMIT 1"
);
mysqli_stmt_bind_param($dup, "iis", $user_id, $state_id, $district_name);
...
$stmt = mysqli_prepare(
    $con,
    "INSERT INTO district (district_name, state_id, population, area_sq_kms, user_id) VALUES (?, ?, ?, ?, ?)"
);
...
mysqli_stmt_bind_param($stmt, "siidi", $district_name, $state_id, $population, $area_sq_kms, $user_id);
