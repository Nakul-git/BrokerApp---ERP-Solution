...
$dup = mysqli_prepare($con, 'SELECT product_id FROM product WHERE user_id=? AND LOWER(product_name)=LOWER(?) LIMIT 1');
mysqli_stmt_bind_param($dup, 'is', $user_id, $product_name);
...
$cols = ['product_name', 'sales_rate', 'rate', 'user_id'];
$vals = [sql_val($con, $product_name, 'str'), sql_val($con, $sales_rate, 'num'), sql_val($con, $rate, 'num'), sql_val($con, $user_id, 'int')];

foreach ($extra as $col => $meta) {
    if (!isset($product_cols[$col])) continue;
    $cols[] = $col;
    $vals[] = sql_val($con, $meta['value'], $meta['type']);
}

mysqli_begin_transaction($con);

try {
    $sql = 'INSERT INTO product (' . implode(',', $cols) . ') VALUES (' . implode(',', $vals) . ')';
    ...
    if (!empty($brand_ids) && table_exists($con, 'product_brand')) {
    ...
    if (!empty($packing_rows) && table_exists($con, 'product_packing_detail')) {
