...
$dup = mysqli_prepare(
    $con,
    'SELECT group_id FROM group_master WHERE user_id=? AND LOWER(group_name)=LOWER(?) AND group_id<>? LIMIT 1'
);
mysqli_stmt_bind_param($dup, 'isi', $user_id, $group_name, $group_id);
...
$stmt = mysqli_prepare(
    $con,
    'UPDATE group_master SET sort_order=?, main_group_name=?, group_name=?, group_type=?, maintain_bill_outstanding=?, suppress_trial_balance=?, address_details_req=?, general_ledger=? WHERE group_id=? AND user_id=?'
);
...
mysqli_stmt_bind_param($stmt, 'isssssssii', $sort_order, $main_group_name, $group_name, $group_type, $maintain_bill_outstanding, $suppress_trial_balance, $address_details_req, $general_ledger, $group_id, $user_id);
